...
public class BlockRoot : MonoBehaviour {
	...
	public static Vector3 calcBlockPosition(Block.iPosition i_pos) {
		Vector3 position = new Vector3 (-(Block.BLOCK_NUM_X / 2.0f - 0.4f),
		                                -(Block.BLOCK_NUM_Y / 2.0f - 1.0f), 0.0f);
	...
	private bool is_grabbable_all() {
		bool ret = true;
		foreach (BlockControl block in this.blocks) {
			if (block.isGrabbable() == false) {
				ret = false;
				break;
			}
		}
